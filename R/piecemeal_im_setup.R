...
setup <- function(df, type = "indicator"){
  
  #subset to just key variables and their disaggs
    df <- df %>% 
  ...
  #list of key disaggs to keep (by indicator)
    tokeep <- 
      tibble::tribble(
  ...
  #inner join to just keep key variables
    df <- inner_join(df, tokeep, by = c("indicator", "disaggregate"))
      rm(tokeep)
  
  
  if(type == "site"){
      #keep distinct list of sites, IMs and their indicators
      df <- df %>% 
        distinct(operatingunit, psnu, facility, mechanismid, indicator) %>% 
        #unite (so one column for location like with variable tables)
        unite(site, psnu, facility, sep = "/", remove = TRUE) %>% 
        #add a field to denote “in site” - X
        mutate(reporting = "X")
    } else {
    #combine all relvent disagg categories into a new var - agesexother
      df <- df  %>% 
    ...
    #summarize to hvae only one psnu x ind by row
      df <- df %>% 
    ...
    #order for later reshape, changing agesexother to ordered factor
